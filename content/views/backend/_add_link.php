...
<script type="text/javascript">
    jQuery.extend({
        parseQuerystringss: function(href){
            var nvpair = {};
    ...
    $(function () {    
        $('table td img[src="images/icon_edit.gif"]').each(function(){
                ...
                var qs = jQuery.parseQuerystringss(href);

                if(href.search("product.php") > -1)
                    parent.before('<a class="link-video" href="<?php echo riLink('rimeta_list')?>?objects_id=' + qs['pID'] + '&objects_type=product">Metas</a>');
        });
    });
</script>
